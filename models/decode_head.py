...
class BaseDecodeHead(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        loss_decode_without_type = {k: v for k, v in loss_decode.items() if k != "type"}
        self.loss_decode = globals()[loss_decode_type](**loss_decode_without_type)
    ...
    def _transform_inputs(self, inputs):
        ...
        if self.input_transform == 'resize_concat':
            ...
            if self.transform_scale is None:
                transform_size = inputs[0].shape[2:]
            else:
                transform_size = tuple([item * self.transform_scale for item in inputs[0].shape[2:]])
            upsampled_inputs = [
                resize(
                    input=x,
                    size=transform_size,
