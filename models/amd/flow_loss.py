...
class unFlowLoss(nn.modules.Module):
    ...
    def forward(self, output, target):
        ...
        if self.cfg.w_real_smooth > 0:
            smooth_loss = self.cfg.w_real_smooth * sum(pyramid_smooth_losses)
            # warp_loss is around 0.1 to 0.2
            # smooth_loss is around 0.0008 to 0.0017 (after multiplying 50 as the smooth factor)
            
            # print(f"Warp loss: {warp_loss}, smooth loss: {smooth_loss}")
