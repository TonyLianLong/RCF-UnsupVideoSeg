...
def original_load_args(filename):
    """Load yaml into args. Doesn't support recursive base_filename.

    Only merges dictionary. Lists will be replaced.

    Args:
        filename (str): overwrite yaml filename
    ...
    base_filename = os.path.join(os.path.dirname(filename), config["base_config"])
    overwrite_filename = filename

    logger.info(f"Loading base config {base_filename} and overwrite config {overwrite_filename}")

    with open(base_filename, "r") as f:
        base_config = yaml.load(f, Loader=UniqueKeyLoader)
...
if __name__ == "__main__":
    original_args = original_load_args("configs/v2.2/amd_v2.2.yaml")
    new_args = load_args("configs/v2.2/amd_v2.2.yaml")
    
    assert original_args == new_args

    original_args = original_load_args("configs/v2.2/amd_v2.2_debug_pretrained.yaml")
    new_args = load_args("configs/v2.2/amd_v2.2_debug_pretrained.yaml")
    
    assert original_args == new_args

    new_args = load_args("configs/v2.2/amd_v2.2.1_debug_pretrained.yaml")
