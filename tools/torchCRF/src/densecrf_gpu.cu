...
namespace dcrf_cuda
{
    ...
    template <int M>
    DenseCRFGPU<M>::DenseCRFGPU(int N) : DenseCRF(N)
    {
    ...
    void DenseCRFGPU<M>::setUnaryEnergy(const float *unaryGPU)
    {
...
#pragma unroll
    ...
    void DenseCRFGPU<M>::setUnaryEnergyFromLabel(const short *labelGPU, float *confidences)
    {
        ...
        for (int i = 0; i < 2 * M; ++i)
        {
            if (i < M)
            {
                np_energies[i] = -log((1.0f - confidences[i]) / (M - 1));
            }
            else
            {
                np_energies[i] = -log(confidences[i - M]);
            }
        }
...
#pragma unroll
    ...
    void DenseCRFGPU<M>::buildMap()
    {
        ...
        if (!map_)
            cudaMalloc((void **)&map_, sizeof(short) * N_);
        dim3 blocks((N_ - 1) / BLOCK_SIZE + 1, 1, 1);
        dim3 blockSize(BLOCK_SIZE, 1, 1);
