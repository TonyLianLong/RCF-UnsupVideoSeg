...
parser.add_argument('--sequences', type=str, default='all', help='sequence to eval')
...
# Ignore precomputed results
if False and os.path.exists(csv_name_global_path) and os.path.exists(csv_name_per_sequence_path):
    ...
    sum_num_frames = 0
    for x in seq_names:
        current_seq_J = J['M_per_object'][x]
        current_seq_len = J['seq_len'][x]
        sum_J += current_seq_J * current_seq_len
        sum_num_frames += current_seq_len
    J_frame_avg = sum_J / sum_num_frames

    # Generate dataframe for the general results
    g_measures = ['J&F-Mean', 'J-Mean', 'J-FrameMean', 'J-Recall', 'J-Decay', 'F-Mean', 'F-Recall', 'F-Decay']
    final_mean = (np.mean(J["M"]) + np.mean(F["M"])) / 2.
    g_res = np.array([final_mean, np.mean(J["M"]), J_frame_avg, np.mean(J["R"]), np.mean(J["D"]), np.mean(F["M"]), np.mean(F["R"]),
                      np.mean(F["D"])])
